...
const render = (elements) => {
  ...
  elements.map((e, idx) => {
    ...
    newTd.innerHTML = `
        ...
        <span class="  py-1 px-3 rounded-full text-base">${e.brief}</span>
        </td>
        <td class="text-center text-black">${e.status !== "notChoosed" ? e.status : "not yet"
      }</td>
      <td class="py-3 px-6 text-center">
      <span class=" py-1 px-3 rounded-full text-base">${e.date}</span>
        </td>
        <td class="p-2 flex justify-center text-black">
        <button class="p-1 bg-blue-400 rounded-md  text-black px-3 focus:scale-95 btnReset" id="${e.id}">Reset</button>
        </td>
        `;
