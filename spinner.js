...
const render = async (elements) => {
    ...
    await elements.map((e) => {
        if (e.status === "notChoosed") {
            ...
            newSpin.innerHTML = `
            <div class="flex w-48 rounded-3xl px-4 h-38 scale-50 text-base flex-col  shadow-lg shadow-white p-3 ${(e.id == random) ? 'bg-blue-400 text-white h-38 -mt-2' : 'bg-white'} ${(e.status != 'notChoosed') ? 'bg-gray-200' : ''}" id="id_${e.id}" >
